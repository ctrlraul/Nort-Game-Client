...
static func from_file(path: String) -> Result:
	...
	if parse_error != OK:
		return Result.err("Failed to parse json: %s at line %s" % [json.get_error_message(), json.get_error_line()])
...
static func to_file(path: String, data, indent = "") -> Result:
	...
	if create_dir_error != OK:
		return Result.err("Failed to write file '%s': %s" % [path, error_string(create_dir_error)])
