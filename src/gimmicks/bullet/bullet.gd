...
func _process(_delta: float) -> void:
	if target:
		__gfx.global_rotation = global_position.angle_to(target.global_position)
	else:
...
func __update_target() -> void:
	...
	var areas = __area.get_overlapping_areas()
...
func _on_area_2d_area_exited(area: Area2D) -> void:
	var craft_body: CraftBody = area.owner
	if craft_body.craft.faction != part.craft.faction:
		__update_target()
